...
print('{0}(OSIC)模擬 , N={1} , soft = {2}'.format(constellation_name, N, soft))
# 定義way為路徑搜尋的方式
...
for k in range(2):
    for i in range(len(snr_db)):
        ...
        for j in range(N):
            ...
            # 定義OSIC detection
            def OSIC_detection(H, y, optimal_detection, index, N, Es, No, constellation):
                # H是通道傳輸矩陣                                                                           (會隨著遞迴而改變)
                # y是接收端收到的向量                                                                    (會隨著遞迴而改變)
                # optimal_detection是detection後的結果
                # index為一個陣列，用來紀錄H的每個行向量是第幾個行向量   (會隨著遞迴而改變)
                # N代表目前接收向量的長度
                # Es代表symbol 能量、雜訊No
                # constellation為星座圖，也就是向量中元素的值域

                # 首先要對H的每個行向量取norm平方並大到小排序
                norm = [[0]*3 for i in range(len(index))]
                for i in range(len(index)):
                    norm[i][0] = index[i]                               # 代表實際上這是第index[i]個行向量
                    norm[i][1] = i                                      # 代表目前第i個行向量
                    # 接下來計算該行向量的norm平方值
                    norm_value = 0
                    for j in range(N):
                        norm_value += abs(H[j,i])**2
                    norm[i][2] = norm_value                             # 代表這是第i個行向量的norm平方值
                norm.sort(key = lambda cust: cust[2], reverse = True)  # 進行排序

                # norm平方最大的行向量所對應到的ZF detection元素，便是該層OSIC detection後的結果
                detection = Es * (Es * H.getH() * H + No * np.identity(H.shape[1])).I * H.getH() * y
                # detection[norm[0][1], 0] 即為該次ZF detection後需要的元素
                # 接下來對detection[norm[0][1], 0]做demapping，我們會得到detect
                min_value = 1000000
                for i in range(len(constellation)):
                    if abs(constellation[i] - detection[norm[0][1], 0]) < min_value:
                        min_value = abs(constellation[i] - detection[norm[0][1], 0])
                        detect = constellation[i]

                optimal_detection[norm[0][0], 0] = detect  # detct 的值域為constellation星座點的集合
                y -= detect * H[:,norm[0][1]]              # 更新接收向量

                if len(index)-1 > 0:
                    # 將本來通道矩陣H中的第norm[0][1]行去除
                    H_new = np.matrix([[0j]*(len(index)-1) for i in range(N)])
                    k = 0
                    ...
                    # 接下來要更新index，代表去除了某個行向量
                    for i in range(len(index)):
                        if(index[i] == norm[0][0]):
                            del index[i]
                            break

                    OSIC_detection(H_new, y, optimal_detection, index, N, Es, No, constellation)
            ...
            if k == 0:
                ...
                OSIC_detection(H_new, y_new, s, [0,1,2,3], 2*Nt, Es, No, constellation_new)
    ...
    if k == 0:
        ...
        if N >= 1000000:  # 在很多點模擬分析的情況下，錯誤率較正確，我們可以將數據存起來，之後就不用在花時間去模擬
            if way == 3:  # 若採用BFS搜尋
                with open('sphere decoding  for {0}(OSIC), soft = {1}, {2}, K={3} (Nt={4}, Nr={5}).dat'.format(constellation_name, soft, way_name, K1, Nt, Nr), 'w') as f:
                    f.write('snr_db\n')
                ...
                with open('sphere decoding  for {0}(OSIC), soft = {1}, {2} (Nt={3}, Nr={4}).dat'.format(constellation_name, soft, way_name, Nt, Nr),'w') as f:
                    f.write('snr_db\n')
                    for m in range(len(snr_db)):
                        f.write("{0} ".format(snr_db[m]))
                    f.write('\nber\n')
