    ...
    if k == 0:
        ...
        if N >= 1000000:  # 在很多點模擬分析的情況下，錯誤率較正確，我們可以將數據存起來，之後就不用在花時間去模擬
            if normalize == 0:   # 若接收端不對雜訊normalize
                if way == 3:    # 若採用BFS搜尋
                    with open('sphere decoding  for {0}(dynamic algorithm)(non-normalize), soft _vector= {1}, {2}, K={3} (Nt={4}, Nr={5}).dat'.format(constellation_name, soft_vector, way_name, K1, Nt, Nr), 'w') as f:
                        f.write('snr_db\n')
                    ...
                    with open('sphere decoding  for {0}(dynamic algorithm)(non-normalize), soft _vector= {1}, {2} (Nt={3}, Nr={4}).dat'.format(constellation_name, soft_vector, way_name, Nt, Nr),'w') as f:
                        f.write('snr_db\n')
                        for m in range(len(snr_db)):
                            f.write("{0} ".format(snr_db[m]))
                            if m < len(snr_db)-1:
                ...
                if way == 3:  # 若採用BFS搜尋
                    with open('sphere decoding  for {0}(dynamic algorithm)(normalize), soft _vector= {1}, {2}, K={3} (Nt={4}, Nr={5}).dat'.format(constellation_name, soft_vector, way_name, K1, Nt, Nr), 'w') as f:
                        f.write('snr_db\n')
                        for m in range(len(snr_db)):
                            f.write("{0} ".format(snr_db[m]))
                            if m < len(snr_db)-1:
                    ...
                    with open('sphere decoding  for {0}(dynamic algorithm)(normalize), soft _vector= {1}, {2} (Nt={3}, Nr={4}).dat'.format(constellation_name, soft_vector, way_name, Nt, Nr),'w') as f:
                        f.write('snr_db\n')
                        for m in range(len(snr_db)):
                            f.write("{0} ".format(snr_db[m]))
                            if m < len(snr_db)-1:
