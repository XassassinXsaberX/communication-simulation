...
#當ofdm symbol發生delay一個單位時間，相當於一個circular shift
def shift(x):
    x_new = [0]*len(x)
    ...
    for i in range(len(snr)):
        ...
        for j in range(N):
            if k==1 :
                ...
                x = np.fft.ifft(X) * Nfft / np.sqrt(Nusc)  # 乘上Nfft/np.sqrt(Nusc)可將一個OFDM symbol的總能量normalize成 1* Nfft
                ...
                # 如果沒有乘上(Nfft+n_guard)/Nfft ， 那麼有加cyclic prefix 跟沒加cyclic prefix，所耗去的能量都是相同的-->不合理
                # ex
                # 若時域信號向量Nfft = 4個點為 [ 1, -1, 1, 1 ]
                # 加上cyclic prefix後變為Nfft個點為 [ 1, -1, 1, 1 ]   Ng = 1個點，所以時域信號向量即為[ 1, 1, -1, 1, 1 ]
                # 在接收端扣除cyclic prefix後時域信號向量變回 [ 1, -1, 1, 1 ]，我們會觀察加上cp跟沒加cp都耗去相同能量，不合理！
                ...
                ######################################################################################################
                #以上為傳送端
                # snr[i] 代表的是平均每個bit的能量，而snr_Es則是代表調變成OFDM symbol後(有Nfft+n_guard個取樣點)每一個取樣點的平均能量
                snr_Es = snr[i] * (Nusc / Nfft) * (Nfft / (Nfft + n_guard))
                No = 1 / snr_Es  # 這裡以假定每一個取樣點的能量為1
                ...
                Y = np.fft.fft(y) * np.sqrt(Nusc) / Nfft  # 現在將y轉到頻域，變成Y
